package code._4_student_effort._2_OO_Challenge._2_Movie;
...
public class Database {
    ...
    public static void StudiosWithMoreThan2Movies(){
        for(Studio studio : getData()){
            if(studio.filme.length > 2){
                System.out.println(studio.nume);
            } else {
                System.out.println("There are no studios with more than 2 videos");
            }
        }
    }

    public static void StudiosWithActorName(){
        for (Studio studio : getData()) {
    ...
    public static void StudiosWithActorAgeMoreThan50(){
        for (Studio studio : getData()) {
            for (Film film : studio.filme) {
                for (Actor actor : film.actori) {
                    if (actor.varsta > 50) {
    ...
    public static void main(String[] args) {
        StudiosWithMoreThan2Movies();
        StudiosWithActorName();
        StudiosWithActorAgeMoreThan50();
    }
}
